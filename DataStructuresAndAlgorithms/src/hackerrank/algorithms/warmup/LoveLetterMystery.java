...
public class LoveLetterMystery {
	static int findOpsToConvertIntoPalindrome(String input) {
		int pivot1 = 0;
		int pivot2 = 0;
		int numOfOps = 0;

		// Check the even/odd length.
		if (input.length() % 2 == 0) {
			pivot1 = input.length() / 2 - 1;
			pivot2 = pivot1 + 1;
		} else {
			pivot1 = input.length() / 2;
			pivot2 = -1;
		}

		if (pivot2 == -1) { // if odd
			for (int i = 1; i <= input.length() / 2; i++) {
				if (input.charAt(pivot1 - i) == input.charAt(pivot1 + i)) {
					continue;
				} else {
					if (input.charAt(pivot1 - i) < input.charAt(pivot1 + i))
						numOfOps += (int) input.charAt(pivot1 + i)
								- (int) input.charAt(pivot1 - i);
					else
						numOfOps += (int) input.charAt(pivot1 - i)
								- (int) input.charAt(pivot1 + i);
				}
			...
			for (int i = 0; i < input.length() / 2; i++) {
				if (input.charAt(pivot1 - i) != input.charAt(pivot2 + i)) {
					if (input.charAt(pivot1 - i) > input.charAt(pivot2 + i))
						numOfOps += (int) input.charAt(pivot1 - i)
								- (int) input.charAt(pivot2 + i);
					else
						numOfOps += (int) input.charAt(pivot2 + i)
								- (int) input.charAt(pivot1- i);
				}
			}
		}
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < numOfInputs; i++) {
			...
			System.out.println(findOpsToConvertIntoPalindrome(input));
		}
